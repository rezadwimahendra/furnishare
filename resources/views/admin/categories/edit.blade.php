...
@section('title', 'Edit Kategori - Furnishare Admin')
...
@section('content')
    <div class="mb-10">
        ...
        <h1 class="serif-logo text-3xl font-bold text-gray-900">Ubah Kategori: {{ $category->name }}</h1>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden p-8 max-w-2xl">
        <form action="{{ route('admin.categories.update', $category->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div>
                ...
                <input type="text" id="name" name="name" value="{{ old('name', $category->name) }}" required placeholder="Contoh: Kursi Santai" class="w-full bg-gray-50 border border-gray-200 focus:border-[#C5A880] focus:ring-1 focus:ring-[#C5A880] rounded-lg px-4 py-3 text-xs text-gray-800 focus:outline-none">
                @error('name')
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="4" placeholder="Tuliskan penjelasan singkat mengenai ruang lingkup kategori produk ini." class="w-full bg-gray-50 border border-gray-200 focus:border-[#C5A880] focus:ring-1 focus:ring-[#C5A880] rounded-lg px-4 py-3 text-xs text-gray-800 focus:outline-none leading-relaxed">{{ old('description', $category->description) }}</textarea>
                @error('description')
                    <p class="text-[10px] text-red-500 mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                <label class="block text-xs font-semibold text-gray-700 uppercase tracking-wider mb-2">Gambar Sampul Kategori</label>
                
                @if($category->image)
                    <div class="h-28 w-28 rounded-lg overflow-hidden border border-gray-100 bg-gray-50 mb-3">
                ...
                <p class="text-[9px] text-gray-400 mt-1">Biarkan kosong jika tidak ingin mengubah gambar. Mendukung format JPEG, PNG, JPG, WEBP. Maksimal ukuran 2MB.</p>
                @error('image')
                    <p class="text-[10px] text-red-500 mt-1">{{ $message }}</p>
                @enderror
            </div>
